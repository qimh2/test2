...
//	public static ResultSet resultSet;//结果集对象
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("function gotoPageNow(pageCount){" +
				...
				" 							window.location.href = '/UserManager2/ManagerUsers?pageNow='+pageNow;" +
				"			    }else{" +
				"" +
				"					alert('跳转到的页，已超过总页数');" +
				"" +
		...
		out.println("欢迎XX登录");
		out.println("<a href = '/UserManager2/MainFrame'>返回主界面</a>");
		out.println("<a href = '/UserManager2/LoginServlet'>安全退出</a>");
...
//			System.out.println("sql 执行成功.......");
			...
			for(Users u:users){
				...
				out.println("<tr><th>"+id+"</th><th>"+username+"</th><th>"+grade+"</th><th>"+emial+"</th><th><a  onClick = 'return confirmoper();' href = '/UserManager2/DelClServlet?id="+id+"&type=delUser'>删除用户</th><th><a href = '/UserManager2/DelClServlet?id="+id+"&type=modUser&username="+URLEncoder.encode(username, "utf-8")+"&pwd="+URLEncoder.encode(password, "utf-8")+"&email="+URLEncoder.encode(emial, "utf-8")+"'>修改用户</th><tr>");
			...
			//显示上一页
			if((pageNow - 1) > 0){
				out.println("<a href = '/UserManager2/ManagerUsers?pageNow="+(pageNow - 1)+"'>上一页</a>");
			}
			
			//显示分页
			for(int i = 0;i < pageCount;i++){
				out.println("<a href = '/UserManager2/ManagerUsers?pageNow="+(i+1)+"'>   <"+(i+1)+"> </a>&nbsp&nbsp");
			}
			if((pageNow+1) <= pageCount){
				out.println("<a href = '/UserManager2/ManagerUsers?pageNow="+(pageNow+1)+"'>下一页</a>");
			}
